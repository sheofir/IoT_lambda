...
public class QtyInstanceOnline implements RequestHandler<S3Event, RestResult> {
		...
		try {
			...
			for(Map<String, AttributeValue> map : listItems){
				if(map.containsKey(Constants.DEVICE_STATUS) 
						&& Constants.DEVICE_STATUS_ONLINE.equals(map.get(Constants.DEVICE_STATUS).getS())){
					onlineDevices ++;
				}
			}
			resultMap.put(Constants.QTY_INSTANCE, allDevices + "");
			resultMap.put(Constants.QTY_INSTANCE_ONLINE, onlineDevices + "");
			resultMap.put(Constants.QTY_INSTANCE_OFFLINE, (allDevices-onlineDevices) + "");
